    ...
    while bids and asks:
        ...
        while volume_to_match > 0:
            ...
            if match.volume <= volume_to_match:  # We totally match.
                ...
                fraction = old_div(volume_to_match, match.volume)
                ...
                old_fraction = old_div((match.volume - volume_to_match), match.volume)
...
def open_position_trades(open_position_offset, db, start_time, volume_currency='BTC', strategy_actor='Multi'):
    ...
    ratio = old_div(volume_to_keep, last_trade.volume)
...
def exchange_profit_participation(profit_units):
    ...
    for unit in profit_units:
        for exchange_name in unit['exchanges']:
            ...
            exchange_profit += old_div(unit['profit'], 2)

            exchange_absolute_profits[exchange_name] = exchange_profit

    exchange_percentage_profits = {}

    for exchange_name, exchange_profit in list(exchange_absolute_profits.items()):
        exchange_percentage_profits[exchange_name] = old_div(exchange_profit, total_profit)
...
def rsb(revenue, total_volume):
    ...
    if total_volume > 0:
        real_spread_per_bitcoin = old_div((revenue.amount), total_volume.amount)
...
def fsb(profit, total_volume):
    ...
    if total_volume > 0:
        fee_spread_per_bitcoin = old_div((profit.amount), total_volume.amount)
