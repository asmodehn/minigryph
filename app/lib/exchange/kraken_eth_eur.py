...
from lib.exchange import order_types
...
class KrakenETHEURExchange(ExchangeAPIWrapper):
    def __init__(self, session=None, configuration=None):
        super(KrakenETHEURExchange, self).__init__(session, configuration)

        # Immutable properties and endpoints.
        self.name = u'KRAKEN_ETH_EUR'
        self.friendly_name = u'Kraken ETH-EUR'
        self.base_url = 'https://api.kraken.com/0'
        self.price_decimal_precision = 1
        self.currency = u'EUR'
        self.volume_currency = u'ETH'
        self.bid_string = 'buy'
        ...
        self.min_order_size = Money('0.002', 'ETH')
        self.max_tick_speed = 5
        self.use_cached_orderbook = False

        # TODO: Unclear if these should be configurable or not.
        self.withdrawal_fee = Money('0.001', 'ETH')
        self.eth_credit_limit = Money('0', 'ETH')
    ...
    def req(self, req_method, url, **kwargs):
        req = super(KrakenETHEURExchange, self).req(req_method, url, **kwargs)
    ...
    def resp(self, req):
        response = super(KrakenETHEURExchange, self).resp(req)
    ...
    def get_trades_info_from_ledger(self, trade_ids, order_open_timestamp, order_close_timestamp):
        ...
        for trade_id in trade_ids:
            trades_info[trade_id] = {
                'eth': Money.loads('ETH 0'),
                'eth_fee': Money.loads('ETH 0'),
                'fiat': Money(0, self.currency),
                'fiat_fee': Money(0, self.currency),
            }

        for ledger_id, entry in list(entries.items()):
            ...
            if currency == 'ETH':
                trades_info[trade_id]['eth'] += Money(amount, 'ETH')
                trades_info[trade_id]['eth_fee'] += Money(entry['fee'], 'ETH')
            else:
    ...
    def closed_orders_resp(self, req):
        ...
        for order_id, raw_order in list(response['closed'].items()):
    ...
    def load_creds(self):
        try:
            ...
            self.api_key = self._load_env('KRAKEN_ETH_EUR_API_KEY')
            self.secret = self._load_env('KRAKEN_ETH_EUR_API_SECRET')
    ...
    def get_balance_resp(self, req):
        ...
        # currency.
        supported_currencies = ['ETH', self.currency]
    ...
    def get_ohlc_from_position_resp(self, req, position):
        ...
        series["hl2"] = old_div((series["high"] + series["low"]), 2)
    ...
    def get_full_ohlc_resp(self, req):
        # TODO return dask dataframe and not pandas
        response = self.resp(req)
        ...
        df["hl2"] = old_div((df["high"] + df["low"]), 2)

        # # Dask
        # name = 'full_ohlc_df'
        # columns = ["timestamp", "open", "high", "low", "close", "vwap", "volume", "count"]
        # df = dd.DataFrame(ohlc, name, columns, divisions=None)
            ...
            if order_id in multi_trades:
                ...
                if trade_ids:
                    ...
                    for t_id, t in list(trades.items()):
                        ...
                        if t['eth_fee'] and t['fiat_fee']:
                            raise exceptions.ExchangeAPIErrorException(
                                self,
                                '#%s charged fees in both fiat (%s) and ETH (%s)' % (
                                order_id,
                                t['fiat_fee'],
                                t['eth_fee'],
                            ))
    ...
    def withdraw_crypto_req(self, address, volume):
        # ! PROBABLY DOESNT WORK FOR ETH
        if not isinstance(address, basestring):
            raise TypeError('Withdrawal address must be a string')

        if self.volume_currency != 'ETH':
            raise TypeError('Kraken withdrawals are only supported for ETH right now.')
        ...
        deposit_addresses = {
            name: addr
            for name, addr in list(os.environ.items()) if '_DEPOSIT_ADDRESS' in name
        }

        address_to_name_map = {
            addr: name.replace('_DEPOSIT_ADDRESS', '')
            for name, addr in list(deposit_addresses.items())
        }
    ...
    def construct_pair(cls, fiat_currency):
        eth_currency_code = cls.convert_to_kraken_currency('ETH')
        fiat_currency_code = cls.convert_to_kraken_currency(fiat_currency)

        return eth_currency_code + fiat_currency_code

    @classmethod
    def convert_from_kraken_currency(cls, kraken_currency):
        if kraken_currency == 'XETH':
            currency = 'ETH'
