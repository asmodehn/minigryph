...
import lib.metrics.midpoint as midpoint_lib                                 #pyx file
import lib.metrics.volume_available as available_volume
from lib.exchange.consts import Consts                                      #pyx file


def orderbook_strength_at_slippage(orderbook, order_type, slippage, volume_currency):
    """
...
def orderbook_strength_at_slippages(orderbook, order_type, slippages):
    ...
    levels = {
        ...
        for price, value in list(levels.items())
    }

    return levels


def orderbook_strength_at_slippage_in_usd(orderbook, order_type, slippage, volume_currency):
    """
    Get the amount of liquidity (bitcoin denominated) available within [slippage]
    dollars of the top [order_type].
