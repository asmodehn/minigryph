...
from execution.console import Trade, Transaction 

import lib.exchange.exchange_factory as exchange_factory
...
def table_entries_from_transaction(transaction):
    ...
    for entry in entries:
        ...
        entry['details'] = ''.join([
            '%s:%s ' % (k, v)
            for k, v in list(transaction.transaction_details.items())
