...
from execution.lib import auditing
from execution.harness.exchange_coordinator import ExchangeCoordinator              #pyx file
from lib import configuration as config_lib
from lib.configurable_object import ConfigurableObject
...
class Harness(ConfigurableObject):
    ...
    def sleep_time_to_next_tick(self):
        ...
        # Convert tick sleep to an integer as by default we parse conf args into Decimal
        # (maybe we should change this directly in lib.configuration)
        return int(self.strategy.tick_sleep)
    ...
    def wind_down(self):
        ...
        assert all([len(orders) == 0 for orders in list(current_orders.values())])
    ...
    def log_orders(self, current_orders):
        ...
        for exchange_name, exchange_orders in list(current_orders.items()):
