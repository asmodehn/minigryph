...
from execution.lib import conf
...
def get_destination_exchange(algo, exchange_data, exchanges):
    ...
    exchanges.sort(key=lambda e: (old_div(full_balances[e.name]['BTC'], rules[e.name]['target'])))
...
def send_btc_from_exchange_to_exchange(source_exchange, destination_exchange, amount):
...
def bitcoin_rebalance(db, exchange_data, algo, execute=False):
    ...
    if execute:
        ...
        deposit_address, transaction_hash, exchange_withdrawal_id = send_btc_from_exchange_to_exchange(source_exchange, destination_exchange, transfer_amount)
